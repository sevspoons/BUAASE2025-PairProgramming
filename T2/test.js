...
export function greedy_snake_barriers_checker(
  initial_snake,
  food_num,
  foods,
  barriers,
  ...
  while (turn <= 200) {
    const direction = greedySnakeMoveBarriers(
      current_snake,
      current_foods,
      barriers
    );
    ...
    // hit a barrier
    if (
      barriers_list.some((ob) => ob.x === new_snake[0] && ob.y === new_snake[1])
    )
      return -2;
    ...
    for (let i = 0; i < current_foods.length; i += 2) {
      if (
        new_snake[0] === current_foods[i] &&
        new_snake[1] === current_foods[i + 1]
      ) {
        ate_index = i;
        break;
      }
