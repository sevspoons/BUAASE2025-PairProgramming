...
import { GAME_MODE, snakeModules, CUSTOM_SEED } from "./game-config.js";
...
console.log(
  `Starting ${TEST_MODE} mode with ${gameState.snake_num} snakes, board size ${gameState.n}x${gameState.n}, ${gameState.food_num} foods, ${gameState.max_rounds} rounds`
);
...
function printBoardState(gameState) {
  ...
  console.log(
    `Living snakes ${alive.filter(Boolean).length}/${snake_num}: ${alive.join(
      ", "
    )}`
  );
...
for (let i = 0; i < gameState.snake_num; i++) {
  console.log(
    `Snake ${i + 1}: ${finalResults.scores[i]} points${
      finalResults.alive[i]
        ? " (survived)"
        : " (died in round " + finalResults.dead_round[i] + ")"
    } spent ${finalResults.time[i].toFixed(3)}ms`
  );
}
